  ...
  for (num_iter = 1; num_iter <= nb_iter; num_iter++)
    {
	  ...
	  */
      ...
      for (i = 0; i < n; ++i) {
          D += (degres_arbre_relax[i] -2) * (degres_arbre_relax[i]-2);
      }

      double t = alpha * (borne - valeur)/D;
      for (i = 0; i < n; ++i) {
          lambda[i] = lambda[i] + t * (degres_arbre_relax[i]-2); 
      }
